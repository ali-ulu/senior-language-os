...
const defaultState={day:1,sessionDone:[],savedSentences:[],errors:[],completedDays:[],activePack:null,missionDone:false,recallStats:[],personalLexicon:[],recallSchedule:{},conversationHistory:[],lastEvaluation:null,speakingStats:[],benchmarks:{baseline:null,final:null}};
let state=loadState(),pack=null,selected={},drillIndex=0,recallIndex=0,listeningIndex=0,scenarioIndex=0,timerInterval=null,timerRemaining=120,timerTotal=120,recallStartedAt=null,activeRecallKey=null,mediaRecorder=null,recordedChunks=[],recordingBlob=null,recordingUrl=null;
...
function bindActions(){
 ...
 $('#errorForm').onsubmit=addError;$('#packInput').onchange=loadPackFile;$('#downloadState').onclick=downloadState;$('#captureBaseline').onclick=()=>captureBenchmark('baseline');$('#captureFinal').onclick=()=>captureBenchmark('final');
}
...
function resetTimer(sec){clearInterval(timerInterval);timerInterval=null;timerRemaining=sec;timerTotal=sec;$('#toggleTimer').textContent='Başlat';drawTimer()}
function drawTimer(){const m=Math.floor(timerRemaining/60),s=timerRemaining%60;$('#timer').textContent=`${pad(m)}:${pad(s)}`}
function toggleTimer(){if(timerInterval){clearInterval(timerInterval);timerInterval=null;$('#toggleTimer').textContent='Devam et';return}$('#toggleTimer').textContent='Duraklat';timerInterval=setInterval(()=>{timerRemaining--;drawTimer();if(timerRemaining<=0){clearInterval(timerInterval);timerInterval=null;$('#toggleTimer').textContent='Tur tamamlandı';state.speakingStats.push({day:state.day,seconds:timerTotal,at:new Date().toISOString(),scenario:conversationScenario()});state.speakingStats=state.speakingStats.slice(-200);persist();toast('Tur tamamlandı. Aynı görevi yeniden yap.') }},1000)}
...
function renderProgress(){$('#phases').innerHTML=(pack.phases||[]).map(p=>`<div class="phase ${phaseForDay(state.day)===p.id?'active':''}"><span>${esc(p.days)}</span><strong>${esc(p.name)}</strong></div>`).join('');$('#dayGrid').innerHTML=Array.from({length:30},(_,i)=>i+1).map(d=>`<button class="day ${d===state.day?'current':''} ${state.completedDays.includes(d)?'complete':''}" data-day="${d}">${pad(d)}</button>`).join('');document.querySelectorAll('#dayGrid [data-day]').forEach(b=>b.onclick=()=>{state.day=Number(b.dataset.day);state.sessionDone=[];state.missionDone=false;state.conversationHistory=[];state.lastEvaluation=null;persist();renderAll();showView('home')});renderBenchmarks()}
...
function benchmarkSnapshot(){
 const recalls=state.recallStats.filter(x=>x.day===state.day&&Number.isFinite(x.seconds));const speeds=recalls.map(x=>x.seconds),med=median(speeds);const speaking=state.speakingStats.filter(x=>x.day===state.day).map(x=>x.seconds);
 return {capturedDay:state.day,at:new Date().toISOString(),recallMedian:med,under3Rate:speeds.length?Math.round(speeds.filter(x=>x<=3).length/speeds.length*100):null,recallAttempts:speeds.length,activeErrors:state.errors.filter(x=>!x.resolved).length,personalExpressions:state.personalLexicon.length,longestSpeakingSeconds:speaking.length?Math.max(...speaking):null};
}
function openBenchmarkDB(){return new Promise((resolve,reject)=>{const req=indexedDB.open('senior-language-os-audio-v1',1);req.onupgradeneeded=()=>{if(!req.result.objectStoreNames.contains('audio'))req.result.createObjectStore('audio')};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error)})}
async function saveBenchmarkAudio(slot,blob){if(!blob||!('indexedDB'in window))return false;try{const db=await openBenchmarkDB();await new Promise((resolve,reject)=>{const tx=db.transaction('audio','readwrite');tx.objectStore('audio').put(blob,slot);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});db.close();return true}catch{return false}}
async function loadBenchmarkAudio(slot){if(!('indexedDB'in window))return null;try{const db=await openBenchmarkDB();const blob=await new Promise((resolve,reject)=>{const tx=db.transaction('audio','readonly'),req=tx.objectStore('audio').get(slot);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error)});db.close();return blob}catch{return null}}
async function captureBenchmark(slot){
 const expected=slot==='baseline'?1:30;if(state.day!==expected){toast((slot==='baseline'?'Baseline':'Final')+' snapshot yalnızca Gün '+expected+' üzerinde alınır.');return}
 const snap=benchmarkSnapshot(),audioSaved=await saveBenchmarkAudio(slot,recordingBlob);snap.audioSaved=audioSaved;state.benchmarks={...(state.benchmarks||{}),[slot]:snap};persist();renderBenchmarks();toast((slot==='baseline'?'Day 1 baseline':'Day 30 final')+' kaydedildi'+(audioSaved?' + ses':''));
}
function formatMetric(v,suffix=''){return v==null?'—':`${typeof v==='number'&&Number.isFinite(v)?Math.round(v*10)/10:v}${suffix}`}
function benchmarkMetricHTML(snap){if(!snap)return '<div class="empty">Henüz snapshot yok.</div>';return `<div><span>Recall medyanı</span><strong>${formatMetric(snap.recallMedian,' sn')}</strong></div><div><span>&lt;3 sn oranı</span><strong>${formatMetric(snap.under3Rate,'%')}</strong></div><div><span>Recall denemesi</span><strong>${formatMetric(snap.recallAttempts)}</strong></div><div><span>En uzun konuşma</span><strong>${formatMetric(snap.longestSpeakingSeconds,' sn')}</strong></div><div><span>Aktif hata</span><strong>${formatMetric(snap.activeErrors)}</strong></div><div><span>Kişisel ifade</span><strong>${formatMetric(snap.personalExpressions)}</strong></div>`}
async function attachBenchmarkAudio(slot,id){const el=$(id);if(!el)return;const blob=await loadBenchmarkAudio(slot);if(!blob){el.classList.add('hidden');return}if(el.dataset.objectUrl)URL.revokeObjectURL(el.dataset.objectUrl);const url=URL.createObjectURL(blob);el.dataset.objectUrl=url;el.src=url;el.classList.remove('hidden')}
function deltaValue(a,b,unit=''){if(a==null||b==null)return '—';const d=Math.round((b-a)*10)/10;return `${d>0?'+':''}${d}${unit}`}
function renderBenchmarks(){
 const b=state.benchmarks?.baseline||null,f=state.benchmarks?.final||null;$('#baselineMetrics').innerHTML=benchmarkMetricHTML(b);$('#finalMetrics').innerHTML=benchmarkMetricHTML(f);$('#captureBaseline').disabled=state.day!==1;$('#captureFinal').disabled=state.day!==30;
 $('#benchmarkDelta').innerHTML=b&&f?`<div><span>Recall medyanı Δ</span><strong>${deltaValue(b.recallMedian,f.recallMedian,' sn')}</strong></div><div><span>&lt;3 sn oranı Δ</span><strong>${deltaValue(b.under3Rate,f.under3Rate,' puan')}</strong></div><div><span>En uzun konuşma Δ</span><strong>${deltaValue(b.longestSpeakingSeconds,f.longestSpeakingSeconds,' sn')}</strong></div><div><span>Aktif hata Δ</span><strong>${deltaValue(b.activeErrors,f.activeErrors)}</strong></div>`:'<div class="empty">Karşılaştırma için Day 1 ve Day 30 snapshot’larının ikisi de gerekli.</div>';
 attachBenchmarkAudio('baseline','#baselineAudio');attachBenchmarkAudio('final','#finalAudio');
}
...
async function loadPackFile(e){const f=e.target.files?.[0];if(!f)return;try{const data=JSON.parse(await f.text());validatePack(data);pack=data;state.activePack=data;state.day=1;state.sessionDone=[];state.completedDays=[];state.recallSchedule={};state.conversationHistory=[];state.lastEvaluation=null;state.speakingStats=[];state.benchmarks={baseline:null,final:null};persist();drillIndex=recallIndex=listeningIndex=scenarioIndex=0;renderAll();toast(`${data.meta.name} yüklendi`)}catch(err){toast(`Paket açılamadı: ${err.message}`)}finally{e.target.value=''}}
